...
Návod na napojení **MojeID** a **Seznam.cz** jako přihlašovacích metod (Identity Providerů) do 
Cloudflare Access, součásti platformy [Cloudflare One](https://developers.cloudflare.com/cloudflare-one/)
(dřív Cloudflare Zero Trust), pomocí Cloudflare Workers.
Oficiální dokumentace obou služeb tohle přímo nepokrývá a člověk narazí na pár nečekaných problémů — tohle
repo popisuje, jak se přes ně dostat.

- **[mojeid/](mojeid/)** — tenký proxy worker (žádné úložiště, žádné závislosti)
- **[seznam/](seznam/)** — plný OIDC shim (Seznam žádný OIDC nemá)
...
ne OpenID Connect — žádný `id_token`, žádné JWKS, žádné discovery. Tady musí worker sehrát roli
...
1. Zaregistrovat klienta (MojeID: `https://mojeid.cz/consumer_admin/`, Seznam: `https://vyvojari.seznam.cz/oauth/admin`) s
   redirect URI `https://<váš-tým>.cloudflareaccess.com/cdn-cgi/access/callback`.
...
3. V Cloudflare One dashboardu přidat nový OpenID Connect login method, Auth/Token/Certificate URL
   nasměrovat na worker (u MojeID jen Auth URL, Token/Certificate zůstávají přímo na `mojeid.cz`).
